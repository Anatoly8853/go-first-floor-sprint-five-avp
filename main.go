...
func (r Running) Calories() float64 {
	lStep := r.meanSpeed()
	sStep := CaloriesMeanSpeedMultiplier*lStep + CaloriesMeanSpeedShift
	dTime := r.Duration.Hours() * MinInHours

	return sStep * (r.Weight / MInKm) * dTime
}
...
func (w Walking) Calories() float64 {
	...
	return ((CaloriesWeightMultiplier*w.Training.Weight + (math.Pow(w.meanSpeed()*KmHInMsec, 2)/w.Height)*CaloriesSpeedHeightMultiplier*w.Training.Weight) * float64(w.Duration.Hours()) * MinInHours)
...
// Формула расчета:
// длина_бассейна * количество_пересечений / м_в_км / продолжительность_тренировки надо рисать что время в часах
// Это переопределенный метод Calories() из Training.
